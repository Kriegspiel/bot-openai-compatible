...
- **Independent Cadence**: keep active-game state polling at its configured
  interval while moving `/game/mine/active` discovery to a separate
  ten-second default.
- **Jitter**: randomize each discovery delay by 15 percent to prevent model
  instances from settling into synchronized request bursts.
- **Compatibility**: retain `--poll-seconds` for active-game runners and add
  explicit discovery interval and jitter options.
